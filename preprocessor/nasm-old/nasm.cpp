    ...
    std::shared_ptr<reaver::assembler::utils::include_chain> inc) const
{
    if (inc->size > 1024)
    {
        ...
        _engine.push(exception(logger::fatal) << "maximum include (or macro) depth reached.");
    }
    ...
    while (std::getline(input, buffer) && (current_line += new_lines))
    {
        ...
        while (buffer.size() && buffer.back() == '\\')
        {
            ...
            if (!std::getline(input, next))
            {
                ...
                _engine.push(exception(logger::error) << "invalid `\\` at the end of file.\n" << v.back() << "\n" << style::style()
...

        {
            ...
            {
                _engine.push(chain()->exception());
                _engine.push(exception(logger::crash) << "%if directive is not implemented yet.\n" << style::style() <<
...

        {
            ...
            if (elif)
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "invalid %elif directive.");
                }

                _engine.push(chain()->exception());
                _engine.push(exception(logger::crash) << "%elif directive is not implemented yet.");
            }
        }

        {
            ...
            if (else_match)
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "invalid %else directive.");
                }
...

        {
            ...
            if (endif)
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "invalid %endif directive.");
                }
            }
        }
...

        {
            ...
            if (define)
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "define name `" << define->name << "` redefined.");
                }
...

        {
            ...
            if (include)
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "junk after %include directive.");
                }
...

        {
            ...
            if (undef)
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "junk after %undef directive.");
                }

                if (state.defines.find(*undef) == state.defines.end())
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "unknown define: `" << *undef << "`.");
                }
...

        {
            ...
            if (begin->type() == assembler::pp_directive && begin->as<std::string>() == "%error")
            {
                ...
                _engine.push(chain()->exception());
                _engine.push(exception(logger::logger::error) << error);
            }
        }

        {
            ...
            {
                _engine.push(chain()->exception());
                _engine.push(exception(logger::error) << "unknown preprocessor directive `" << begin->as<std::string>() << "`.");
            }
            ...
            while (begin != tokens.cend())
            {
                ...
                {
                    _engine.push(chain()->exception());
                    _engine.push(exception(logger::error) << "invalid place for a preprocessor directive `" << begin->as<std::string>()
                        << "`.");
                }
    ...
    reaver::assembler::nasm_preprocessor_state & state, std::shared_ptr<reaver::assembler::utils::include_chain> chain) const
{
    ...
    while (i < tokens.size())
    {
        ...
        if (state.defines.find(begin->as<std::string>()) != state.defines.end())
        {
            ...
            {
                _engine.push(chain->exception());
                _engine.push(exception(logger::error) << "define `" << begin->as<std::string>() << "` defined recursively.");
            }
            ...
            if (define->parameters().size())
            {
                ...
                if (match)
                {
                    ...
                    {
                        _engine.push(chain->exception());
                        _engine.push(exception(logger::error) << "wrong number of parameters for define `" << define->name()
                ...
                {
                    _engine.push(chain->exception());
                    _engine.push(exception(logger::error) << "no parameters supplied for macro `" << define->name() << "` taking "
